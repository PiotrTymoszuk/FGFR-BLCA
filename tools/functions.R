...
# Markdown helpers ---------
  ...
  number_figures <- function(figure_lst) {
    ...
    ## sets consecutive numbers in labels of the figure objects
    ## in a list

    for(i in seq_along(figure_lst)) {

      if(stri_detect(figure_lst[[i]]$label, regex = '^figure_')) {
      ...
      figure_lst[[i]]$label <-
        paste0('figure_', i, '_', figure_lst[[i]]$label)
  ...
  save_tables <- function(tab_lst, path, ...) {

    ## saves a list of mdtables pn the disc

    cover <- tab_lst %>%
      map_chr(attr, 'caption')

    cover <-
      tibble(Table = paste('Table', 1:length(tab_lst)),
             Caption = cover)

    tab_lst <- tab_lst %>%
      set_names(paste('Table', 1:length(tab_lst)))

    tab_lst <- c(list(Cover = cover), tab_lst)

    tab_lst %>%
      write_xlsx(path = path, ...)
