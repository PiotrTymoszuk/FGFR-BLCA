...
flextable::flextable(suppl_tabs$cohorts) %>% 
  set_widths(c(2.7, rep(2.3, 6), 2.3, 2)) %>% 
  footnote(1, 8:9, 
...
flextable::flextable(suppl_tabs$genetic_alterations) %>% 
  ...
  merge_v(3) %>% 
  italic(j = 3) %>% 
  set_widths(c(2.5, 3, 2.2, 3, rep(2.5, 3))) %>% 
  theme_vanilla
...
flextable::flextable(suppl_tabs$sub_drugs) %>% 
  set_widths(c(1.6, 2.2, 1.6, rep(2.9, 4), 2.3, 2)) %>% 
  merge_v(1) %>% 
  footnote(1, 3, 
           as_paragraph('IC50: concentration in µM resulting in 50% inhibition of growth; AUC: area under the dose-response curve.'), 
           part = 'header', 
           ref_symbols = 'a') %>% 
  footnote(1, 8:9, 
           value = as_paragraph('Statistical significance: Kruskal-Wallis test with \u03B7\u00B2 effect size statistic. P values corrected for multiple testing with the false discovery rate method.'), 
           part = 'header', 
           ref_symbols = 'b') %>% 
  theme_vanilla
